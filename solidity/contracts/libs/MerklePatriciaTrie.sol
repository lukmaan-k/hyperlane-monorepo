...
/**
 * @title MerkleTrie
 * SOURCE: https://github.com/HerodotusDev/trie-proofs/blob/main/examples/eth/src/lib/MerkleTrie.sol
 * Simpler to user than lib/fx-portal/contracts/libs/MerklePatriciaProof.sol
 * No need for hex-prefix encoding for each node in stack
 */
library MerklePatriciaTrie {
    ...
    function getSingleNodeRootHash(
        ...
        return
            keccak256(
                _makeLeafNode(BytesUtils.toNibbles(_key), _value).encoded
            );
    }
        ...
        )
    {
        ...
        for (uint256 i = 0; i < _proof.length; i++) {
            ...
            if (currentNode.decoded.length == BRANCH_NODE_LENGTH) {
                ...
                bytes memory keyRemainder = BytesUtils.slice(
                    key,
                    currentKeyIndex
                );
        ...
        return (
            pathLength,
            BytesUtils.slice(key, currentKeyIndex),
            isFinalNode
        );
    }
    ...
    function _getNewPath(
        ...
        if (keyRemainder.length == 0 && lastNodeType == NodeType.LeafNode) {
            ...
            if (keyRemainder.length == 0) {
                ...
                // to our branch node.
                newNodes[totalNewNodes] = _makeLeafNode(
                    BytesUtils.slice(keyRemainder, 1),
                    _value
                );
                totalNewNodes += 1;
            }
            ...
            if (sharedNibbleLength != 0) {
                ...
                bytes memory nextNodeKey = BytesUtils.slice(
                    lastNodeKey,
                    0,
                    sharedNibbleLength
                );
    ...
    function _parseProof(
        ...
        for (uint256 i = 0; i < nodes.length; i++) {
            ...
            proof[i] = TrieNode({
                encoded: encoded,
                decoded: RLPReader.readList(encoded)
            });
        }
    ...
    function _makeNode(
        ...
        return
            TrieNode({encoded: encoded, decoded: RLPReader.readList(encoded)});
    }
    ...
    function _editBranchValue(
        ...
        _branch.decoded[_branch.decoded.length - 1] = RLPReader.toRLPItem(
            encoded
        );
