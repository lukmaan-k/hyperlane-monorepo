...
/**
 * @title MPTProof
 * @dev Library for verifying Merkle-Patricia-Trie proofs.
 *      >>> SOURCE: https://github.com/ibc-solidity/solidity-mpt/blob/main/src/MPTProof.sol <<<
 *      Simpler to use compared to library found in:
 *      lib/fx-portal/contracts/lib/MerklePatriciaProof.sol
 *      No need to hex-prefix encode the trie key at each node in the stack
 */
library MPTProof {
        ...
        for (uint256 i = 0; i < proof.length; i++) {
            ...
            if (node.length == 2) {
                ...
                if (mptKeyOffset != mptKeyNibbles.length) {
                    ...
                    uint256 nibble = uint256(
                        uint8(mptKeyNibbles[mptKeyOffset])
                    );
                    mptKeyOffset += 1;
                    if (nibble >= 16) {
    ...
    function mptHashHash(
        RLPReader.RLPItem memory input
